...
async function getInstruction(f) {
    ...
    if (!cat) return null;
    let data = await loadJSON(`js/${cat === 'it' ? 'it' : cat}.json`);
    return data?.[f] || null;
}
...
async function showModal(file, addTitle = null) {
    ...
    try {
        ...
        $('#modalBody').innerHTML = instr?.content?.length ? renderContent(instr.content) : '<div style="color:red">⚠️ Инструкция в разработке</div>';
    } catch (e) { $('#modalBody').innerHTML = '<div style="color:red">⚠️ Ошибка загрузки</div>'; }
}
...
function showAppsModal() {
    let modal = $('#appsModal');
    $('#appsModalBody').innerHTML = Object.values(DB.apps).map(a => `<div class="card" style="margin-bottom:12px"><h4>📱 ${esc(a.name)}</h4><p>${esc(a.description)}</p><a href="${a.downloadUrl}" target="_blank" class="btn btn-primary" style="display:inline-block;margin-top:12px">Скачать</a></div>`).join('');
...
async function loadItData() {
    ...
    itData = Object.entries(data || {}).map(([id, ins]) => ({ id, title: getTitle(id), desc: getDesc(id), category: ins.category || 'stores' }));
}

const categoryNames = { stores: '🏪 Магазины', sales_agents: '🚀 Торговые агенты', servers: '🖥️ Серверы', local_settings: '⚙️ Локальные настройки' };
...
function renderItDash() {
    ...
    if (itFilter === 'all' && !itSearch) {
        let html = '';
        for (let [k, name] of Object.entries(categoryNames)) {
        ...
        container.innerHTML = `<div class="it-cards-grid">${filtered.map(i => `<div class="it-card" data-file="${i.id}"><div class="it-card-category">${categoryNames[i.category] || '📄'}</div><div class="it-card-title">📄 ${esc(i.title)}</div><div class="it-card-desc">${esc(i.desc)}</div><span class="card__link">Открыть →</span></div>`).join('')}</div>`;
    }
...
function setupItFilters() {
    let filters = [{ id: 'all', name: '📋 Все' }, { id: 'stores', name: '🏪 Магазины' }, { id: 'sales_agents', name: '🚀 Торговые' }, { id: 'servers', name: '🖥️ Серверы' }, { id: 'local_settings', name: '⚙️ Локальные' }];
    $('#itFilters').innerHTML = filters.map(f => `<button class="filter-btn ${itFilter === f.id ? 'active' : ''}" data-filter="${f.id}">${f.name}</button>`).join('');
    ...
    $('#itSearchInput').addEventListener('input', e => { itSearch = e.target.value; $('#itClearSearch').style.display = itSearch ? 'block' : 'none'; renderItDash(); });
